...
**Neighbors:** `FormationSlot.column` is a `float`, authored explicitly (not derived from `position.x`). Same-row slots use whole numbers (`0, 1, 2, …`); a row with fewer slots that's interleaved between another row (e.g. 2 slots behind a row of 3) uses half-steps (`0.5, 1.5, …`) so each of its slots sits — and counts as adjacent — between the two slots of the other row it's behind/in front of. `EnemyFormation.get_neighbors(enemy: Enemy) -> Array[Enemy]` (via `_are_adjacent`) returns live (`hp > 0`) enemies that are either same-row with `column` off by exactly `1`, or one row apart (`|row diff| == 1`) with `column` off by exactly `0.5`. Example (`back_heavy`: 3 back at columns 0/1/2, 2 front at 0.5/1.5): front slot 0.5's neighbors are front 1.5 (same row), back 0 and back 1 (both 0.5 away) — not back 2. Back slot 0's neighbors are back 1 (same row) and front 0.5 — not front 1.5. Prefer the convenience wrapper `Enemy.get_neighbors() -> Array[Enemy]` from spell effects / statuses rather than reaching into `CombatContext.formation` directly. This is groundwork for effects that should also affect the target's neighbors (e.g. a cleave `DealDamageEffect` variant, or a status that spreads via `on_apply`) — write it as its own `SpellEffectResource`/`StatusEffect` hook, do not special-case neighbors inside `CombatManager`.
